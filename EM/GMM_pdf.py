...
if __name__ == '__main__':
    ...
    grid_plot = -gmm.score_samples(grid_test)
    grid_plot = grid_plot.reshape(x1.shape)
    plt.figure(figsize=(9, 7), facecolor='w')
    ax = plt.subplot(111)
    cme = plt.pcolormesh(x1, x2, grid_plot, cmap=plt.cm.Spectral)
    plt.colorbar(cme, shrink=1)
    CS = plt.contour(x1, x2, grid_plot, levels=np.logspace(0, 2, num=levels, base=10), colors='w', width=1)
